...
void LMS::dbInteract::fill_datagrid_member(System::Windows::Forms::DataGridView^ dataGridView)
{
	...
	MySqlCommand^ select_all_members_cmdDataBase = gcnew MySqlCommand("SELECT member_id AS ID, member_name AS Name, member_mobile AS Mobile, member_email as 'E-Mail', member_no_book_stat AS 'No. of Books Borrowed' FROM library_system_db.member_data WHERE membership_stat = 'ACTIVATED';", conDataBase);

	try {
		MySqlDataAdapter^ datagrid_adapter = gcnew MySqlDataAdapter();
		datagrid_adapter->SelectCommand = select_all_members_cmdDataBase;
		DataTable^ database_dataset = gcnew DataTable();
		datagrid_adapter->Fill(database_dataset);
		BindingSource^ source_bind = gcnew BindingSource();
		source_bind->DataSource = database_dataset;
		dataGridView->DataSource = source_bind;
		datagrid_adapter->Update(database_dataset);
	}
	catch (Exception^ ex)
	{
...
void LMS::dbInteract::fill_datagrid_members_filtered(String^ str_list_detail_search_person, String^ search_bar_text, System::Windows::Forms::DataGridView^ dataGridView)
{
	...
	MySqlCommand^ show_filtered_member_cmdDataBase = gcnew MySqlCommand("SELECT member_id AS ID, member_name AS Name, member_mobile AS Mobile, member_email as 'E-Mail', member_no_book_stat AS 'No. of Books Borrowed' FROM library_system_db.member_data\
		WHERE " + str_list_detail_search_person + " LIKE '%" + search_bar_text + "%' AND membership_stat = 'ACTIVATED';", conDataBase);

	try {
		MySqlDataAdapter^ datagrid_adapter = gcnew MySqlDataAdapter();
		datagrid_adapter->SelectCommand = show_filtered_member_cmdDataBase;
		DataTable^ database_dataset = gcnew DataTable();
		datagrid_adapter->Fill(database_dataset);
		BindingSource^ source_bind = gcnew BindingSource();
		source_bind->DataSource = database_dataset;
...
void LMS::dbInteract::fill_datagrid_book(System::Windows::Forms::DataGridView^ dataGridView)
{
	...
	MySqlCommand^ select_all_books_cmdDataBase = gcnew MySqlCommand("SELECT book_id AS ID, book_name as Title, book_author AS Author, book_edition_no AS Edition, book_publisher AS Publisher, book_borrow_status AS Status FROM library_system_db.book_data WHERE NOT book_borrow_status = 'LOST';", conDataBase);

	try {
		MySqlDataAdapter^ datagrid_adapter = gcnew MySqlDataAdapter();
		datagrid_adapter->SelectCommand = select_all_books_cmdDataBase;
		DataTable^ database_dataset = gcnew DataTable();
		datagrid_adapter->Fill(database_dataset);
		BindingSource^ source_bind = gcnew BindingSource();
		source_bind->DataSource = database_dataset;
...
void LMS::dbInteract::fill_datagrid_books_filtered(String^ str_list_detail_search_book, String^ search_bar_text, System::Windows::Forms::DataGridView^ dataGridView)
{
	...
	MySqlCommand^ show_filtered_books_cmdDataBase = gcnew MySqlCommand("SELECT book_id AS ID, book_name as Title, book_author AS Author, book_edition_no AS Edition, book_publisher AS Publisher, book_borrow_status AS Status FROM library_system_db.book_data\
		WHERE " + str_list_detail_search_book + " LIKE '%" + search_bar_text + "%' AND NOT book_borrow_status = 'LOST';", conDataBase);

	try {
		MySqlDataAdapter^ datagrid_adapter = gcnew MySqlDataAdapter();
		datagrid_adapter->SelectCommand = show_filtered_books_cmdDataBase;
		DataTable^ database_dataset = gcnew DataTable();
		datagrid_adapter->Fill(database_dataset);
		BindingSource^ source_bind = gcnew BindingSource();
		source_bind->DataSource = database_dataset;
...
void LMS::dbInteract::fill_datagrid_borrow_history(System::Windows::Forms::DataGridView^ dataGridView, bool is_librarian_input, String^ transfer_id_input)
{
	...
	if (is_librarian_input == true)
	{
		...
		MySqlCommand^ select_all_borrow_cmdDataBase = gcnew MySqlCommand("SELECT order_id AS ID, book_id AS 'Book ID', member_id AS 'Member ID', date_issue AS 'Issue Date', date_returned AS 'Return Date', borrow_fine AS 'Fine', borrow_status AS 'Status' FROM library_system_db.borrow_history;", conDataBase);

		try {
			MySqlDataAdapter^ datagrid_adapter = gcnew MySqlDataAdapter();
			datagrid_adapter->SelectCommand = select_all_borrow_cmdDataBase;
			DataTable^ database_dataset = gcnew DataTable();
			datagrid_adapter->Fill(database_dataset);
			BindingSource^ source_bind = gcnew BindingSource();
			source_bind->DataSource = database_dataset;
	...
	else
	{
		...
		MySqlCommand^ select_member_borrow_cmdDataBase = gcnew MySqlCommand("SELECT order_id AS ID, book_id AS 'Book ID', member_id AS 'Member ID', date_issue AS \
		...
		try {
			MySqlDataAdapter^ datagrid_adapter = gcnew MySqlDataAdapter();
			datagrid_adapter->SelectCommand = select_member_borrow_cmdDataBase;
			DataTable^ database_dataset = gcnew DataTable();
			datagrid_adapter->Fill(database_dataset);
			BindingSource^ source_bind = gcnew BindingSource();
			source_bind->DataSource = database_dataset;
...
void LMS::dbInteract::fill_datagrid_borrow_history_filtered(String^ str_list_detail_search_order, String^ search_bar_text, System::Windows::Forms::DataGridView^ dataGridView, bool is_librarian_input, String^ transfer_id_input)
{
	...
	if (is_librarian_input == true)
	{
		...
		MySqlCommand^ select_all_filtered_borrow_cmdDataBase = gcnew MySqlCommand("SELECT order_id AS ID, book_id AS 'Book ID', member_id AS 'Member ID', date_issue AS \
		...
		try {
			MySqlDataAdapter^ datagrid_adapter = gcnew MySqlDataAdapter();
			datagrid_adapter->SelectCommand = select_all_filtered_borrow_cmdDataBase;
			DataTable^ database_dataset = gcnew DataTable();
			datagrid_adapter->Fill(database_dataset);
			BindingSource^ source_bind = gcnew BindingSource();
			source_bind->DataSource = database_dataset;
	...
	else
	{
		...
		MySqlCommand^ select_member_filtered_borrow_cmdDataBase = gcnew MySqlCommand("SELECT order_id AS ID, book_id AS 'Book ID', member_id AS 'Member ID', date_issue AS \
			'Issue Date', date_returned AS 'Return Date', borrow_fine AS 'Fine', borrow_status AS 'Status' FROM library_system_db.borrow_history\
			WHERE " + str_list_detail_search_order + " LIKE '%" + search_bar_text + "%' AND member_id = " + transfer_id_input + ";", conDataBase);

		try {
			MySqlDataAdapter^ datagrid_adapter = gcnew MySqlDataAdapter();
			datagrid_adapter->SelectCommand = select_member_filtered_borrow_cmdDataBase;
			DataTable^ database_dataset = gcnew DataTable();
			datagrid_adapter->Fill(database_dataset);
			BindingSource^ source_bind = gcnew BindingSource();
			source_bind->DataSource = database_dataset;
...
void LMS::dbInteract::fill_datagrid_member_profile(String^ member_id_input, System::Windows::Forms::DataGridView^ dataGridView)
{
	...
	MySqlCommand^ member_borrow_cmdDataBase = gcnew MySqlCommand("SELECT order_id AS 'Order ID', book_id AS 'Book ID', member_id AS 'Member ID', date_issue AS 'Date of Issue', date_returned AS 'Date of Return', borrow_fine AS 'Fine', borrow_status AS 'Status' FROM library_system_db.borrow_history WHERE member_id = " + member_id_input + ";", conDataBase);

	try {
		MySqlDataAdapter^ datagrid_adapter = gcnew MySqlDataAdapter();
		datagrid_adapter->SelectCommand = member_borrow_cmdDataBase;
		DataTable^ database_dataset = gcnew DataTable();
		datagrid_adapter->Fill(database_dataset);
		BindingSource^ source_bind = gcnew BindingSource();
		source_bind->DataSource = database_dataset;
...
void LMS::dbInteract::fill_datagrid_book_profile(String^ book_name_input, System::Windows::Forms::DataGridView^ dataGridView, String^ book_edition_input)
{
	...
	MySqlCommand^ fill_copy_book_cmdDataBase = gcnew MySqlCommand("SELECT book_id AS 'Book ID', book_name AS 'Name', book_author AS 'Author', book_edition_no AS 'Edition No', book_publisher AS 'Publisher', book_borrow_status AS 'Borrow Status' FROM library_system_db.book_data \
			WHERE book_name = '" + book_name_input + "' AND book_edition_no = " + book_edition_input + " AND NOT book_borrow_status = 'LOST';", conDataBase);
	try {
		MySqlDataAdapter^ datagrid_adapter = gcnew MySqlDataAdapter();
		datagrid_adapter->SelectCommand = fill_copy_book_cmdDataBase;
		DataTable^ database_dataset = gcnew DataTable();
		datagrid_adapter->Fill(database_dataset);
		BindingSource^ source_bind = gcnew BindingSource();
		source_bind->DataSource = database_dataset;
